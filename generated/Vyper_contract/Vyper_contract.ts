...
export class Vyper_contract extends ethereum.SmartContract {
  static bind(address: Address): Vyper_contract {
    return new Vyper_contract("Vyper_contract", address);
  }
  ...
  escrows_length(): BigInt {
    let result = super.call("escrows_length", "escrows_length():(uint256)", []);

    return result[0].toBigInt();
  }

  try_escrows_length(): ethereum.CallResult<BigInt> {
    let result = super.tryCall(
      "escrows_length",
      "escrows_length():(uint256)",
      []
    );
    if (result.reverted) {
      return new ethereum.CallResult();
  ...
  try_escrows(arg0: BigInt): ethereum.CallResult<Address> {
